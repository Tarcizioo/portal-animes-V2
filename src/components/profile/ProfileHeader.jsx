...
export function ProfileHeader({ user, profile, onEdit, onShare, readOnly = false }) {
  ...
  return (
    <div className="relative mb-20 md:mb-20">
      {/* Banner */}
      <div className="h-36 md:h-72 w-full rounded-2xl md:rounded-3xl overflow-hidden relative shadow-2xl group">
        <img
          src={banner}
          alt="Banner"
        ...
        <div className="absolute top-3 right-3 md:top-6 md:right-6 flex gap-2">
          <span className="bg-black/40 backdrop-blur-md text-white px-2.5 py-0.5 md:px-3 md:py-1 rounded-full text-[10px] md:text-xs font-medium border border-white/10 flex items-center gap-1">
            <span className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${isOnline ? 'bg-green-400' : 'bg-gray-400'}`}></span> {isOnline ? 'Online' : 'Offline'}
          </span>
        </div>
      </div>
        ...
        <div className="group relative">
          <div className="w-28 h-28 md:w-36 md:h-36 rounded-full p-1 md:p-1.5 bg-background-light dark:bg-background-dark shadow-2xl">
            <img
              src={photo}
              alt={name}
              className="w-full h-full rounded-full object-cover border-3 md:border-4 border-white dark:border-[#1e1e24]"
            />
          </div>
          {!readOnly && (
            <div className="absolute -bottom-1 right-0 md:bottom-2 md:right-2 flex gap-1.5 md:gap-2 z-10">
              {/* Share Button */}
              <button
                onClick={onShare}
                className="p-1.5 md:p-2 bg-bg-tertiary text-text-primary rounded-full shadow-lg hover:bg-bg-secondary transition-all transform hover:scale-110 cursor-pointer border border-border-color"
              ...
              <button
                onClick={onEdit}
                className="p-1.5 md:p-2 bg-primary text-white rounded-full shadow-lg hover:bg-primary-hover transition-all transform hover:scale-110 cursor-pointer"
                title="Editar Perfil"
              >
