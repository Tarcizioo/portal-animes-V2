...
export function PublicProfile() {
    ...
    const { profile: myProfile } = useUserProfile(); // perfil do visitante logado
    const isOwnProfile = currentUser?.uid === uid;

    // ── Notificação de visita de perfil ──────────────────────────────────────
    useEffect(() => {
        if (!loading && profile && currentUser && !isOwnProfile) {
            notifyProfileView(uid, myProfile, currentUser.uid);
        }
    }, [loading, profile, currentUser, isOwnProfile, uid, myProfile]);
