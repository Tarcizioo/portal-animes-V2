...
function SortableSection({ id, isEditing, children }) {
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className={`relative flex gap-3 items-stretch group/section rounded-2xl transition-all ${isDragging ? 'ring-2 ring-button-accent ring-offset-2 ring-offset-bg-primary scale-[1.01]' : ''
                }`}
        >
            {/* Drag Handle */}
            <div
...
export function Profile() {
    ...
    // Aguardar auth + profile + biblioteca antes de mostrar conteúdo real
